...
void BitUtils::SetBits(WireData& pCode, unsigned int pInsertionMSBLimit, unsigned int pInsertionLSBLimit, WireData pToInsert)
{
	ASSERT(pInsertionMSBLimit >= pInsertionLSBLimit, "MSB Limit must be higher or equal to the LSB limit when truncating bits");
